...
class SmallLayers:
    ...
    def __init__(self, network, complexity=0):
        self.network = network
        self.complexity = complexity

    def inference0(self, input_x, is_training=True):
        with tf.variable_scope("inference0"):
            x = conv2d_bn_lrelu(input_x, self.network.cs[1], [4, 4], 2, is_training)
            if self.complexity < 0.5:
                x = conv2d_bn_lrelu(x, self.network.cs[2], [4, 4], 2, is_training)
                x = tf.contrib.layers.flatten(x)
                return tf.contrib.layers.fully_connected(x, self.network.cs[3], activation_fn=tf.identity)
            else:
                return conv2d_bn_lrelu(x, self.network.cs[1], [4, 4], 1, is_training)
